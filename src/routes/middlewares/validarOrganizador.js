const db = require('../config/db');

module.exports = (req, res, next) => {
const { codigo_grupo, codigo_organizador } = req.body;

if (!codigo_grupo || !codigo_organizador) {
    return res.status(400).json({
    erro: "Código do grupo e do organizador são obrigatórios."
    });
}

  // Busca o grupo
db.query(
    "SELECT * FROM grupos WHERE codigo = ?",
    [codigo_grupo],
    (err, results) => {
if (err) {
    return res.status(500).json({ erro: "Erro no servidor." });
    }

if (results.length === 0) {
    return res.status(404).json({ erro: "Grupo não encontrado." });
    }

    const grupo = results[0];

      // Valida organizador
if (grupo.codigo_organizador !== codigo_organizador) {
    return res.status(403).json({
    erro: "Código do organizador inválido."
        });
    }

      // Anexa o grupo à requisição
    req.grupo = grupo;

      // Libera a execução para o controller
    next();
        }
    );
};
